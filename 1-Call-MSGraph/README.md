...
This sample application is a modified version of Micrososft initial sample that shows how to use the [Microsoft identity platform endpoint](http://aka.ms/aadv2) to access the data of Microsoft business customers in a long-running, non-interactive process. It uses the [OAuth 2 client credentials grant](https://docs.microsoft.com/en-us/azure/active-directory/develop/v2-oauth2-client-creds-grant-flow) to acquire an access token, which can be used to call the [Microsoft Graph](https://graph.microsoft.io) and access organizational data

The app is a .NET Core 2.1 Console application. It posts ta news sharepoint (sites) list item in an Azure AD tenant by using `Microsoft Authentication Library (MSAL) for .NET` to acquire a token.

## Scenario

The class library:

The class library isolates the classes in the original microsoft project into a single project that both a console or a web app can reference

The console application:

- gets a token from Azure AD in its own name (without a user)
- and then then calls the Microsoft Graph /sites endpoint to insert a new list item in a sharepoint list
  which it then displays (as Json blob)

The Web application:

- accepts user input in a form
- gets a token from Azure AD in its own name (without a user)
- and then calls the Microsoft Graph /sites endpoint to insert a new list item in a sharepoint list
...
To run the console app or the web app, make the chosen app as the default starter project when using visual studio.

Start the conso application, it will display the result of the operation including the inserted record if successful
