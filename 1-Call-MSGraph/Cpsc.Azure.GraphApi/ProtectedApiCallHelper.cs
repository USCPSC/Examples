...
namespace Cpsc.Azure.GraphApi
{
    ...
    public async Task<string> CallWebApiAndProcessResultASync(HttpRequestMessage httpRequestMessage, string accessToken)
        {
            if (!string.IsNullOrEmpty(accessToken))
            {
                ...
                else
                {
                    ...
                    s.AppendLine($"Failed to call the web API: {response.StatusCode}");
            ...
            {
                return "{error:'accessToken is null}";
            }
        }
    }
}
